...
./build-ca < /home/osmc/OpenVPN-Setup/ca_info.txt
...
sed 's/LOCALIP/'$LOCALIP'/' </home/osmc/OpenVPN-Setup/server_config.txt >/etc/openvpn/server.conf
...
sed 's/LOCALIP/'$LOCALIP'/' </home/osmc/OpenVPN-Setup/firewall-openvpn-rules.txt >/etc/firewall-openvpn-rules.sh
...
sed 's/PUBLICIP/'$PUBLICIP'/' </home/osmc/OpenVPN-Setup/Default.txt >/etc/openvpn/easy-rsa/keys/Default.txt

# Make directory under home directory for .ovpn profiles
mkdir /home/osmc/ovpns
chmod 777 -R /home/osmc/ovpns

# Make other scripts in the package executable
cd /home/osmc/OpenVPN-Setup
